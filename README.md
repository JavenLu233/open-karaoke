...
| 播放器页面 | 准备素材与历史记录 |
| --- | --- |
| ![播放器页面预览](./docs/screenshots/player-preview.png) | ![准备素材与历史记录](./docs/screenshots/asset-inputs.png) |
...
浏览器出于安全原因不会允许网页永久保存并直接读取本地文件路径。项目使用 `localStorage` 保存历史列表信息，并使用 IndexedDB 保存用户授权后的 File System Access API 文件句柄。点击“选择并记忆素材”一次后，就可以把音频、封面和 LRC 保存为一个素材组，之后从历史记录恢复；如果文件被移动、删除或权限被撤回，需要重新选择文件。每条记录还会保存当时的歌名、歌手名和歌词时间偏移值，恢复素材时会一起恢复。历史记录超过 5 条时会出现分页器。
...
当前版本仍是纯客户端工具：素材只在浏览器本地处理，不会上传到服务器。未主动保存的选择不会跨刷新保留；已保存的素材组会出现在历史记录中。
